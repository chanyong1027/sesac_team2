...
class OrganizationMemberControllerTest {
    ...
    class GetMembersApi {
        ...
        void getMembers_Success() throws Exception {
            ...
            List<OrganizationMemberResponse> response = List.of(
                new OrganizationMemberResponse(1L, 1L, "owner@example.com", "오너", OrganizationRole.OWNER, "ACTIVATE", LocalDateTime.now()),
                new OrganizationMemberResponse(2L, 2L, "admin@example.com", "관리자", OrganizationRole.ADMIN, "ACTIVATE", LocalDateTime.now()),
                new OrganizationMemberResponse(3L, 3L, "member@example.com", "멤버", OrganizationRole.MEMBER, "ACTIVATE", LocalDateTime.now())
            );
    ...
    class RemoveMemberApi {
        ...
        void removeMember_Success() throws Exception {
            ...
            OrganizationMemberRemoveResponse response = new OrganizationMemberRemoveResponse(memberId, LocalDateTime.now());

            given(organizationMemberService.removeMember(eq(organizationId), eq(memberId), eq(userId)))
                .willReturn(response);

            // when & then
            mockMvc.perform(delete("/api/v1/organizations/{organizationId}/members/{memberId}", organizationId, memberId)
                    .header("X-User-Id", userId))
                .andExpect(status().isOk())
