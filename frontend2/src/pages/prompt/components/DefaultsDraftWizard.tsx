...
export function DefaultsDraftWizard({ workspaceId, promptId, onComplete }: DefaultsDraftWizardProps) {
    ...
    if (isLoading) {
        return <div className="p-8 text-center text-[var(--text-secondary)]">로딩 중...</div>;
    }
                        ...
                        <button
                            ...
                            className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${currentStep === step
                                    ...
                                    : isSectionCompleted(step)
                                        ...
                                        : 'bg-[var(--input)] text-[var(--text-tertiary)] border-[var(--border)]'
                                }`}
                        >
                            {isSectionCompleted(step) ? '✓' : idx + 1}
                        </button>
                        {idx < 4 && (
                            <div className={`w-12 h-0.5 mx-2 ${isSectionCompleted(step) ? 'bg-emerald-500/50' : 'bg-[var(--border)]'
                                }`} />
                        )}
                    </div>
                ))}
            ...
            <div className="bg-[var(--background-card)] rounded-xl p-6 border border-[var(--border)]">
                {currentStep === 'DATASET' && (
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[var(--foreground)]">1. 데이터셋 선택</h3>
                        <p className="text-sm text-[var(--text-secondary)]">평가에 사용할 기본 데이터셋을 선택하세요.</p>
                        <div className="grid grid-cols-2 gap-3">
                            {datasets?.map((dataset) => (
                                <button
                                    key={dataset.id}
                                    onClick={() => setSelectedDatasetId(dataset.id)}
                                    className={`p-4 rounded-lg border text-left transition-all ${selectedDatasetId === dataset.id
                                            ? 'border-[var(--primary)] bg-purple-500/10'
                                            : 'border-[var(--border)] hover:border-[var(--text-tertiary)]'
                                        }`}
                                >
                                    <div className="font-bold text-sm text-[var(--foreground)]">{dataset.name}</div>
                                    <div className="text-xs text-[var(--text-secondary)] mt-1">{dataset.description || '설명 없음'}</div>
                                </button>
                            ))}
                        </div>
                        <button
                            ...
                            className="w-full py-2 bg-[var(--primary)] hover:bg-[var(--primary-hover)] disabled:bg-[var(--input)] disabled:text-[var(--text-tertiary)] text-white rounded-lg text-sm font-bold"
                        >
                ...
                {currentStep === 'RUBRIC' && (
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[var(--foreground)]">2. 루브릭 선택</h3>
                        <p className="text-sm text-[var(--text-secondary)]">평가 기준 템플릿을 선택하세요.</p>
                        <div className="space-y-2">
                            {RUBRIC_OPTIONS.map((rubric) => (
                                <button
                                    key={rubric.code}
                                    onClick={() => setSelectedRubric(rubric.code)}
                                    className={`w-full p-4 rounded-lg border text-left transition-all ${selectedRubric === rubric.code
                                            ? 'border-[var(--primary)] bg-purple-500/10'
                                            : 'border-[var(--border)] hover:border-[var(--text-tertiary)]'
                                        }`}
                                >
                                    <div className="flex justify-between items-center">
                                        <span className="font-bold text-sm text-[var(--foreground)]">{rubric.label}</span>
                                        {selectedRubric === rubric.code && (
                                            <span className="text-[var(--primary)]">✓</span>
                                        )}
                                    </div>
                                    <div className="text-xs text-[var(--text-secondary)] mt-1">{rubric.description}</div>
                                </button>
                            ))}
                        </div>
                        <div className="flex gap-2">
                            <button
                                ...
                                className="flex-1 py-2 bg-[var(--input)] hover:bg-[var(--hover)] text-[var(--foreground)] rounded-lg text-sm border border-[var(--border)]"
                            >
                                이전
                            </button>
                            <button
                                ...
                                className="flex-1 py-2 bg-[var(--primary)] hover:bg-[var(--primary-hover)] disabled:bg-[var(--input)] disabled:text-[var(--text-tertiary)] text-white rounded-lg text-sm font-bold"
                            >
                ...
                {currentStep === 'MODE' && (
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[var(--foreground)]">3. 평가 모드 선택</h3>
                        <p className="text-sm text-[var(--text-secondary)]">평가 실행 방식을 선택하세요.</p>
                        <div className="space-y-2">
                            {MODE_OPTIONS.map((mode) => (
                                <button
                                    key={mode.value}
                                    onClick={() => setSelectedMode(mode.value)}
                                    className={`w-full p-4 rounded-lg border text-left transition-all ${selectedMode === mode.value
                                            ? 'border-[var(--primary)] bg-purple-500/10'
                                            : 'border-[var(--border)] hover:border-[var(--text-tertiary)]'
                                        }`}
                                >
                                    <div className="flex justify-between items-center">
                                        <span className="font-bold text-sm text-[var(--foreground)]">{mode.label}</span>
                                        {selectedMode === mode.value && (
                                            <span className="text-[var(--primary)]">✓</span>
                                        )}
                                    </div>
                                    <div className="text-xs text-[var(--text-secondary)] mt-1">{mode.description}</div>
                                </button>
                            ))}
                        </div>
                        <div className="flex gap-2">
                ...
                {currentStep === 'AUTOMATION' && (
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[var(--foreground)]">4. 자동 평가 설정</h3>
                        <p className="text-sm text-[var(--text-secondary)]">버전 생성 시 자동으로 평가를 실행할지 설정하세요.</p>
                        <div className="space-y-2">
                            {[
                                ...
                                <button
                                    ...
                                    className={`w-full p-4 rounded-lg border text-left transition-all ${autoEvalEnabled === option.value
                                            ? 'border-[var(--primary)] bg-purple-500/10'
                                            : 'border-[var(--border)] hover:border-[var(--text-tertiary)]'
                                        }`}
                                >
                                    <div className="flex justify-between items-center">
                                        <span className="font-bold text-sm text-[var(--foreground)]">{option.label}</span>
                                        {autoEvalEnabled === option.value && (
                                            <span className="text-[var(--primary)]">✓</span>
                                        )}
                                    </div>
                                    <div className="text-xs text-[var(--text-secondary)] mt-1">{option.desc}</div>
                                </button>
                            ))}
                        </div>
                        <div className="flex gap-2">
                ...
                {currentStep === 'REVIEW' && (
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-[var(--foreground)]">5. 설정 확인 및 저장</h3>
                        <p className="text-sm text-[var(--text-secondary)]">아래 설정을 확인하고 저장하세요.</p>
                        <div className="space-y-3 bg-[var(--input)] rounded-lg p-4 border border-[var(--border)]">
                            <ReviewItem
                                label="데이터셋"
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                className="flex-1 py-2 bg-emerald-500 hover:bg-emerald-600 disabled:bg-[var(--input)] disabled:text-[var(--text-tertiary)] text-white rounded-lg text-sm font-bold"
                            >
    ...
    return (
        <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
                <span className="text-sm text-[var(--foreground)] font-bold">{value}</span>
                {completed && <span className="text-emerald-500 dark:text-emerald-400">✓</span>}
            </div>
        </div>
    );
}
