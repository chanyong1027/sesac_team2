...
- 비용, 지연, 오류를 `traceId` 단위로 추적하기 어려워집니다.
...
```mermaid
graph TD
    A["클라이언트 서비스"] --> B["Gateway API<br/>/v1/chat/completions"]
    H["운영 콘솔<br/>frontend2"] --> C["Spring Boot Backend"]
    B --> C
    C --> D["프롬프트 버전 및 배포"]
    C --> E["재시도 / 페일오버 / 예산 가드"]
    ...
    C --> L["선택형 RAG 파이프라인<br/>Tika + Hybrid Search"]
    L --> I
```
